...
from config import MATRIXTM, bot, call_py, CHANNEL, PHOTO_CH
from MusicMatrix.helpers.queues import QUEUE, add_to_queue, get_queue
...
from MusicMatrix.helpers.merrors import capture_err
...
@Client.on_message(filters.command(["تشغيل"], prefixes=f"{MATRIXTM}"))
...
@Client.on_message(filters.command(["تشغيل_فيديو"], prefixes=f"{MATRIXTM}"))
...
@Client.on_message(filters.command(["اغنية عشوائية"], prefixes=f"{MATRIXTM}"))
async def playfrom(client, m: Message):
    chat_id = m.chat.id
    if len(m.command) < 2:
        await m.reply(
            f"**الاستخدام:** \n\n`{MATRIXTM}اغنية عشوائية [ايدي دردشه/معرفها]` \n`{MATRIXTM}اغنية عشوائية [ايدي دردشه/معرفها]`"
        )
    else:
...
""",                  
            ...
            await hmm.delete()
            await m.reply(  
                   f"➕ يضيف {lmt} أغنية في قائمة الانتظار \n• ارسل {MATRIXTM}التشغيل_التلقائي لاضاف اغنيه في القائمه الانتضار**" 
                         )
        except Exception as e:
